...
namespace Puya.ServiceModel
{
        ...
        where TResponse : ServiceResponse<TData>, new()
    {
        ...
        private async Task DoRun(TRequest request, TResponse response, bool async, CancellationToken cancellation)
        {
            if (async)
            {
                response.Data = await Db.ExecuteSingleCommandAsync<TData>($"usp1_{Owner.Name}_{Name}", request, cancellation);
            }
            else
            {
                response.Data = Db.ExecuteSingleCommand<TData>($"usp1_{Owner.Name}_{Name}", request);
            }
