...
namespace Puya.Extensions
{
    ...
    {
        public static byte? FromHex(this char ch)
        {
            byte? result = null;
            ...
            if (char.IsLetterOrDigit(ch))
            {
                if (char.IsLetter(ch))
                {
                    if (ch >= 65 && ch < 91)
                    {
                        result = (byte)(ch - 65 + 10);
                    }
                    else
                    {
                        result = (byte)(ch - 97 + 10);
                    }
                }
                else
                {
        ...
        public static byte[] FromHex(this string str)
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                {
                    var ch1 = str[i];
                    var ch2 = i + 1 < str.Length ? str[i + 1] : default;
                    var b1 = FromHex(ch1);
                    var b2 = ch2 > 0 ? FromHex(ch2) : null;

                    if (b1.HasValue)
                    {
                        if (b2.HasValue)
                        {
                            bytes.Add((byte)(b1.Value * 16 + b2.Value));
                        }
                        else
                        {
                            bytes.Add(b1.Value);
        ...
        #endregion
        public static bool IsJson(this string x)
        {
            var result = IsJsonObject(x) || IsJsonArray(x);

            return result;
        }
        public static bool IsJsonArray(this string x)
        {
            var result = false;

            if (!string.IsNullOrEmpty(x))
            {
                result = x[0] == '[' && x[x.Length - 1] == ']';
            }

            return result;
        }
        public static bool IsJsonObject(this string x)
        {
            var result = false;
        ...
        public static object SafeDeserialize(this string x, object @default = null)
        {
            var result = @default;
            ...
            if (!string.IsNullOrEmpty(x))
            {
                try
                {
                    result = JsonConvert.DeserializeObject(x);
                }
                catch
                { }
            }
        ...
        public static T SafeDeserialize<T>(this string x, T @default = default)
        {
            ...
            if (!string.IsNullOrEmpty(x))
            {
                try
                {
                    result = JsonConvert.DeserializeObject<T>(x);
                }
                catch
                { }
            }
