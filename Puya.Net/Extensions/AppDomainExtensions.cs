...
namespace Puya.Extensions
{
    public static class AppDomainExtensions
    {
        ...
        public static List<Type> GetImplementationsOf(this AppDomain domain, Type interfacetype)
        {
            var result = (from assembly in domain.GetAssemblies()
                          from t in assembly.GetImplementationsOf(interfacetype)
                          select t).ToList();

            return result;
        }
        public static List<Type> GetImplementationsOf<T>(this AppDomain domain)
        {
            var result = (from assembly in domain.GetAssemblies()
                          from t in assembly.GetImplementationsOf<T>()
                          select t).ToList();
